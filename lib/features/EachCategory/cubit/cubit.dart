...
class CategoryCubit extends Cubit<ShopCategoryStates>{
  ...
  void getCategoryProduct(){
    emit(ShopCategoryLoadingState());
     DioHelper.getData(url: '$Products/${EachCategoryScreen.slug ?? EachCategoryScreen.name}', token: token
    ).then((value)async {
      ...
      for (final element1 in products) {
              ...
              children: [
                ...
                InkWell(
                  onTap: () {
                    ...
                    if (token != null) {
                      CategoryCubit.get(context).toggleFavoriteProduct(token!,model);
                    }

                  },
                  child: model.in_favorites!?const Icon(Iconsax.heart5):const Icon(Iconsax.heart),
                ),
